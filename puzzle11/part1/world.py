...
class World:
    ...
    def code(self):
        codes = []
        codes.append('E' + str(self.elevator_floor))

        codes.extend(sorted([generator.code() for generator in self.generators]))
        codes.extend(sorted([microchip.code() for microchip in self.microchips]))
    ...
    def is_valid(self):
        for floor in range(4):
            ...
            generator_materials_without_microchips = generator_materials.difference(microchip_materials)
            microchip_materials_without_generators = microchip_materials.difference(generator_materials)

            if len(generator_materials_without_microchips) > 0 and len(microchip_materials_without_generators) > 0:
                return False
